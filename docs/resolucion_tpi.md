...
- **Solución implementada:** 
  1. **Checkout del Carrito:** Se diseñó el modal de confirmación en `cart.html` incrustando la selección de Método de Pago. Se agregaron campos bloqueados (`disabled`) para Teléfono y Dirección de entrega, inyectando el número de la sesión del cliente en TypeScript. Esto logra la estética solicitada sin alterar el JSON (`PedidoCreate`) que consume el backend.
...
### Ausencia de Fechas en Carga Inicial y Entidades Nuevas

- **Problema detectado:** Al cargar la base de datos o guardar nuevos productos, el campo `createdAt` o `Create_at` aparecía como `null`, impidiendo llevar un registro cronológico de creación en las entidades.
- **Causa raíz:** La clase abstracta `Base.java` tenía declarada la propiedad `LocalDateTime createdAt;`, pero carecía de las instrucciones de auditoría necesarias para que el ORM (Hibernate) inyectara automáticamente la fecha actual durante el proceso de guardado.
- **Solución implementada:** Se refactorizó el atributo añadiéndole dos anotaciones clave: `@org.hibernate.annotations.CreationTimestamp` (para que Hibernate genere el `LocalDateTime.now()` y lo asigne milisegundos antes del `INSERT` en la BD) y `@jakarta.persistence.Column(updatable = false)` (como medida de seguridad para sellar la columna y que ningún `UPDATE` futuro pueda alterar o vaciar esa fecha original).
